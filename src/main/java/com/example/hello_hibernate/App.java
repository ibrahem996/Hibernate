...
public class App {
	...
	private static void generateCars() throws Exception {
		...
		for (int i = 0; i < 10; i++) {
			...
			session.save(car);
			/*
			 * The call to session.flush() updates the DB immediately without ending the
			 * transaction. Recommended to do after an arbitrary unit of work. MANDATORY to
			 * do if you are saving a large amount of data - otherwise you may get cache
			 * errors.
			 */
			session.flush();
		}
